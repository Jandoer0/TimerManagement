...
ShowActiveTimers() {
    ...
    btnClose.OnEvent("Click", CloseHiddenTimersWindow)
    
    HiddenTimersGui.OnEvent("Close", CloseHiddenTimersWindow)
...
; Функция для закрытия окна скрытых таймеров
CloseHiddenTimersWindow(*) {
...
UpdateHiddenTimersDisplay() {
    ...
    ; Проверяем существование окна безопасным способом
    if (!IsObject(HiddenTimersGui)) {
        return
    }
    
    try {
        ; Пытаемся получить HWND окна - если окно уничтожено, это вызовет ошибку
        hwnd := HiddenTimersGui.Hwnd
        if (!hwnd || !WinExist("ahk_id " hwnd)) {
            CloseHiddenTimersWindow()
            return
        }
    } catch {
        ; Если произошла ошибка при получении HWND, окно уничтожено
        CloseHiddenTimersWindow()
        return
    }
    ...
    ; Если не осталось скрытых таймеров, закрываем окно
    if (HiddenTimersControls.Count = 0) {
        CloseHiddenTimersWindow()
    }
}
...
RemoveTimerFromHiddenList(timerID) {
    ...
    for controlType, control in controls {
        if (IsObject(control)) {
            try control.Destroy()
        }
    }
